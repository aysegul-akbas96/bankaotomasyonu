...
namespace BankaHesapUygulaması_3Nisan2019
{
    public partial class KrediKartı : Form
    {
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("Delete from KrediKartı where KrediKartıID='" + comboBox1.SelectedItem + "'", baglanti);
            komut.ExecuteNonQuery();
            baglanti.Close();
            Listele("select *from KrediKartı");
        }
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("select KrediKartıID from KrediKartı", baglanti);
            SqlDataReader dr;
            dr = komut.ExecuteReader();
            while (dr.Read())
            {
                comboBox1.Items.Add(dr["KrediKartıID"]);
            }
            baglanti.Close();
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("insert into KrediKartı(HesapID,Tipi,Numara) values(@HesapID,@Tipi,@Numara)", baglanti);
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("select *from KrediKartı where KrediKartıID='" + comboBox1.SelectedItem.ToString() + "'", baglanti);
        ...
        private void bütçeGüncelleToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand komut = new SqlCommand("Update KrediKartı set HesapID='" + comboBox2.SelectedItem.ToString() + "',Tipi='" + textBox1.Text.ToString() + "',Numara='" +textBox2.Text.ToString()+ "'where KrediKartıID='" + comboBox1.SelectedItem.ToString() + "'", baglanti);
